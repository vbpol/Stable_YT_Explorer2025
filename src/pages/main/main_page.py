            ...
            def _fetch_playlists():
                    ...
                    for v in videos:
                        ...
                        for pl in chpls:
                            ...
                            if not has:
                                continue
                            if not any(p['playlistId'] == plid for p in collected):
                                collected.append(pl)
                                try:
                                ...
                                try:
                                    def _prefetch(pid):
                                        try:
                                            ...
                                            print(f"[Prefetch] Failed for {pid}: {e}")
                                    import threading as _t_pf
                                    _t_pf.Thread(target=_prefetch, args=(plid,), daemon=True).start()
                                except Exception:
                                    pass
            ...
            for i, item in enumerate(items):
                try:
                    ...
                    if not v:
                        continue
                    vid = v.get('videoId')
                    is_hit = bool(vid) and (self.video_playlist_cache.get(vid) == playlist_id)

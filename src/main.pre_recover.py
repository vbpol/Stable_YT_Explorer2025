    ...
    def _restore_last_state(self):
        try:
            ...
            if last_mode == 'videos':
                ...
                try:
                    data = self.datastore.load_last_videos_result() or {}
                ...
                try:
                    data = self.datastore.load_last_playlists_result() or {'playlists': [], 'query': ''}
                    q = (data.get('query') or '').strip()
                    if q:
                        mp.search.search_entry.delete(0, 'end')
                        mp.search.search_entry.insert(0, q)
                ...
                if vq:
                    ...
                    try:
                        mp.search.search_entry.delete(0, 'end')
                        mp.search.search_entry.insert(0, vq)
                    except Exception:
                        pass
                elif pq:
                    try:
                    ...
                    try:
                        mp.search.search_entry.delete(0, 'end')
                        mp.search.search_entry.insert(0, pq)
                    except Exception:
                        pass
        except Exception:
            pass

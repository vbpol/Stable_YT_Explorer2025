...
class ConfigManager:
    ...
    # --- Last query persistence (for restoring keywords on startup/mode change) ---
    @staticmethod
    def get_last_query_path() -> str:
        try:
            base = ConfigManager.get_data_dir()
            return os.path.join(base, "last_query.json")
        except Exception:
            return "last_query.json"

    @staticmethod
    def save_last_query(query: str, mode: str):
        try:
            path = ConfigManager.get_last_query_path()
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"query": query or "", "mode": (mode or "").strip()}, f, indent=2)
        except Exception:
            pass
    ...
    def get_last_mode_path() -> str:
        try:
            return os.path.join(ConfigManager.get_data_dir(), "last_mode.txt")
        except Exception:
            return "last_mode.txt"

    @staticmethod
    def save_last_mode(mode: str):
        try:
            with open(ConfigManager.get_last_mode_path(), "w", encoding="utf-8") as f:
